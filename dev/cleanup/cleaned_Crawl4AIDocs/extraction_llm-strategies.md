...
8. **`input_format`**(str): Determines**which** crawler result is passed to the LLM. Options include: 
  ...
  3. `"fit_markdown"`: The filtered “fit” markdown if you used a content filter. 
...
> **Tip:** `JsonCssExtractionStrategy.generate_schema()` also supports token usage tracking via an optional `usage` parameter. See [Token Usage Tracking in Schema Generation](https://docs.crawl4ai.com/extraction/no-llm-strategies/#token-usage-tracking) for details.
* * *
